...
class PdfJsService implements PDFService {
    ...
    async buildSchedule(data: { trainings: Prisma.TrainingGetPayload<{include: {excersises: true, schedule: {include: {days: true}}}}>[], owner: string}): Promise<Buffer> {
        ...
        header.cell({ paddingTop: 1 * pdf.cm }).text('HORARIO DE ENTRENAMIENTO', { fontSize: 16, color: black });
        header.cell({ paddingTop: 1 * pdf.cm }).text(null);
        header.cell({ paddingTop: 1 * pdf.cm }).text(new Date().toLocaleDateString(), { fontSize: 16, color: black, textAlign: 'right' });
      ...
      profileHeader.cell().text(data.owner, { fontSize: 14, color: black })
        ...
        const subHeader = doc
          ...
          .row()
        subHeader.cell('', { backgroundColor: gray, textAlign: 'center', })
          .text('ENTRENAMIENTO', { fontSize: 12, textAlign: 'center', color: black });
        subHeader.cell('', { backgroundColor: gray, textAlign: 'center', })
        ...
        for (const training of data.trainings) {
          for (const excercise of training.excersises) {
            const row = doc.table({ widths: [6 * pdf.cm, 6 * pdf.cm, 6 * pdf.cm, null], ...border })
              .row({ padding });
              row.cell().text(training.name, { fontSize: 9,  textAlign: 'center' })
              row.cell().text(excercise.name, {fontSize: 9, textAlign: 'center'})
              row.cell().text(DayTurn[training.schedule.turn], { fontSize: 9, textAlign: 'center' })
              row.cell().text(training.schedule.days.map(day => DayWeek[day.day_of_week]).join(', '), { fontSize: 9, textAlign: 'center' })
          }
        }
