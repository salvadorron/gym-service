...
export class TrainerRepositoryImpl implements TrainerRepository {
    ...
    async getTrainers(): Promise<Trainer[]> {
        const trainers = await this.prisma.trainer.findMany({ include: { certificates: true } });
        return trainers;
    }
    async getTrainerById(id: number): Promise<Trainer> {
        const trainer = await this.prisma.trainer.findUnique({ where: { id }, include: { certificates: true } });
